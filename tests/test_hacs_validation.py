...
class TestHACSValidation:
    ...
    def test_manifest_hacs_requirements(self):
        ...
        assert (
            "_" not in manifest["domain"]
            or manifest["domain"].replace("_", "").isalnum()
        )
    ...
    def test_integration_structure_hacs_compliant(self):
        ...
        required_files = ["__init__.py", "manifest.json", "strings.json"]
    ...
    def test_strings_json_hacs_format(self):
        ...
        assert (
            "title" in user_step or "data" in user_step
        ), "User step should have title or data"
        ...
        for file_name in python_files:
            ...
            if file_path.exists():
                ...
                for pattern in blocking_patterns:
                    ...
                    if pattern == "open(" and (
                        "manifest.json" in content or "strings.json" in content
                    ):
                        continue
        ...
        for method_name in api_methods:
            if hasattr(Loca2ApiClient, method_name):
                ...
                assert inspect.iscoroutinefunction(
                    method
                ), f"API method {method_name} should be async"
    ...
    def test_dependencies_are_minimal(self):
        ...
        for dep in dependencies:
            assert not dep.startswith(
                "custom_components"
            ), f"Should not depend on custom component: {dep}"
        ...
        for req in requirements:
            ...
            if not any(op in req for op in [">=", "<=", "==", "~=", "!="]):
                ...
                if package_name not in allowed_without_version:
                    pytest.skip(
                        f"Consider adding version constraint to requirement: {req}"
                    )
    ...
    def test_device_info_compliance(self):
        ...
        mock_device = Loca2Device(
            id="test_device", name="Test Device", device_type="smartphone"
        )
    ...
    def test_entity_unique_id_format(self):
        ...
        mock_coordinator = Mock()
        mock_device = Loca2Device(
            id="test_device_123", name="Test Device", device_type="smartphone"
        )
        ...
        for file_name in python_files:
            ...
            if file_path.exists():
                ...
                if "import logging" in content or "from logging" in content:
                    assert (
                        "_LOGGER = logging.getLogger(__name__)" in content
                ...
                for i, line in enumerate(lines, 1):
                    if "print(" in line and not line.strip().startswith("#"):
                        assert (
                            False
                        ), f"{file_name}:{i} should use logger instead of print()"
