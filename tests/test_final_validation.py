...
class TestCompleteUserWorkflow:
    ...
    async def test_integration_setup_workflow(
    ...
    async def test_device_discovery_workflow(
        self, hass: HomeAssistant, mock_config_entry, mock_api_client
    ):
...
class TestAsyncImplementationPerformance:
    ...
    async def test_coordinator_update_performance(self, mock_api_client):
        ...
        coordinator = Loca2DataUpdateCoordinator(
            hass=Mock(), api_client=mock_api_client, scan_interval=30
        )
            ...
            if file_path.exists():
                ...
                for pattern in blocking_patterns:
                    ...
                    if pattern.startswith("requests.") or pattern.startswith("httpx."):
                        assert (
                            pattern not in content
                        ), f"Found blocking HTTP operation '{pattern}' in {file_name}"
                    else:
                        assert (
                            pattern not in content
                        ), f"Found blocking operation '{pattern}' in {file_name}"
