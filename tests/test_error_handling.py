...
class TestApiClientErrorHandling:
    ...
    async def test_retry_logic_with_structured_logging(self, api_client, caplog):
        ...
        with aioresponses() as m:
            ...
            attempt_logs = [
                record for record in caplog.records if "attempt" in record.message
            ]
...
class TestCoordinatorErrorHandling:
    ...
    async def test_error_tracking_and_categorization(self, coordinator, caplog):
        ...
        assert (
            diagnostics["error_tracking"]["error_categories"][ERROR_CATEGORY_AUTH] == 1
        )

        # Test network error
        coordinator.api_client.get_devices.side_effect = Loca2ConnectionError(
            "Network failed"
        )
        ...
        diagnostics = coordinator.get_diagnostic_info()
        assert (
            diagnostics["error_tracking"]["error_categories"][ERROR_CATEGORY_NETWORK]
            == 1
        )
    ...
    async def test_user_notifications_for_errors(self, coordinator, mock_hass):
        ...
        coordinator.api_client.get_devices.side_effect = Loca2AuthError(
            "Invalid API key"
        )
    ...
    @pytest.mark.asyncio
    async def test_rate_limit_handling_and_interval_adjustment(
        self, coordinator, caplog
    ):
        """Test rate limit handling and scan interval adjustment."""
        coordinator.api_client.get_devices = AsyncMock()
        coordinator.api_client.get_devices.side_effect = Loca2RateLimitError(
            "Rate limited"
        )
    ...
    async def test_consecutive_error_handling_with_backoff(self, coordinator, caplog):
        ...
        backoff_logs = [
            record for record in caplog.records if "Applying backoff" in record.message
        ]
        assert len(backoff_logs) >= 1

    @pytest.mark.asyncio
    async def test_recovery_notification_and_logging(
        self, coordinator, mock_hass, caplog
    ):
    ...
    async def test_performance_monitoring_slow_updates(self, coordinator, caplog):
        ...
        slow_logs = [
            record for record in caplog.records if "Slow API response" in record.message
        ]
        assert len(slow_logs) >= 1
...
class TestDeviceTrackerErrorHandling:
    ...
    async def test_location_fetch_error_handling(self, mock_device_tracker, caplog):
        ...
        mock_device_tracker.coordinator.async_get_device_location.side_effect = (
    ...
    @pytest.mark.asyncio
    async def test_comprehensive_update_error_logging(
        self, mock_device_tracker, caplog
    ):
        """Test comprehensive error logging during update."""
        # Mock coordinator refresh to fail
        mock_device_tracker.coordinator.async_request_refresh = AsyncMock()
        mock_device_tracker.coordinator.async_request_refresh.side_effect = Exception(
            "Coordinator error"
        )
    ...
    async def test_location_quality_assessment(self, mock_device_tracker):
        ...
        excellent_location = Loca2Location(
        ...
        good_location = Loca2Location(
            latitude=37.7749, longitude=-122.4194, accuracy=25.0
        )
        quality = mock_device_tracker._assess_location_quality(good_location)
        assert quality == "good"

        # Test fair quality location
        fair_location = Loca2Location(
            latitude=37.7749, longitude=-122.4194, accuracy=75.0
        )
        quality = mock_device_tracker._assess_location_quality(fair_location)
        assert quality == "fair"

        # Test poor quality location
        poor_location = Loca2Location(
            latitude=37.7749, longitude=-122.4194, accuracy=150.0
        )
...
class TestCoordinatorHealthChecks:
    ...
    async def test_health_check_stale_data(self, coordinator):
        ...
        coordinator.api_client.test_connection = AsyncMock(return_value=True)
        coordinator._last_successful_update = datetime.now() - timedelta(
            minutes=10
        )  # Very stale
    ...
    async def test_health_check_exception_handling(self, coordinator):
        ...
        coordinator.api_client.test_connection = AsyncMock(
...
class TestStructuredLoggingUtilities:
    ...
    def test_structured_logger_error_logging(self, caplog):
        ...
        error_logs = [
            record for record in caplog.records if "test_error" in record.message
        ]
        assert len(error_logs) >= 1
    ...
    def test_structured_logger_performance_logging(self, caplog):
        ...
        with caplog.at_level(logging.DEBUG):
            ...
            structured_logger.log_performance(
                "very_slow_operation", 12.0, "very slow operation"
            )

        # Verify performance logs
        perf_logs = [
            record for record in caplog.records if "operation" in record.message
        ]
        ...
        very_slow_logs = [
            record for record in caplog.records if "VERY SLOW:" in record.message
        ]
    ...
    def test_operation_timer_context_manager(self, caplog):
        ...
        timer_logs = [
            record for record in caplog.records if "timed_operation" in record.message
        ]
    ...
    def test_diagnostic_collector_error_tracking(self):
        ...
        # Add some errors
        collector.add_error(
            "auth", "invalid_key", "Authentication failed", severity="critical"
        )
        collector.add_error(
            "network", "timeout", "Connection timeout", severity="medium"
        )
...
class TestEnhancedCoordinatorErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_enhanced_notification_with_severity(
        self, coordinator, mock_hass, caplog
    ):
...
class TestEnhancedDeviceTrackerErrorHandling:
    ...
    @pytest.mark.asyncio
    async def test_structured_location_update_logging(
        self, enhanced_device_tracker, caplog
    ):
    ...
    async def test_consecutive_error_tracking(self, enhanced_device_tracker, caplog):
        ...
        enhanced_device_tracker.coordinator.async_get_device_location = AsyncMock()
        enhanced_device_tracker.coordinator.async_get_device_location.side_effect = (
            Exception("Location error")
        )
        enhanced_device_tracker.coordinator.async_request_refresh = AsyncMock()
    ...
    @pytest.mark.asyncio
    async def test_stale_location_clearing_on_persistent_errors(
        self, enhanced_device_tracker, caplog
    ):
        """Test clearing of stale location data on persistent errors."""
        from custom_components.loca2.api import Loca2Location
        ...
        # Set initial location
        enhanced_device_tracker._location = Loca2Location(
            latitude=37.7749, longitude=-122.4194
        )
...
class TestConfigFlowErrorHandling:
    ...
    async def test_options_validation_comprehensive_logging(self, caplog):
        ...
        success_logs = [
            record for record in caplog.records if "validation passed" in record.message
        ]
