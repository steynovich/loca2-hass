...
from custom_components.loca2.const import CONF_BASE_URL, DEFAULT_SCAN_INTERVAL, DOMAIN
...
class TestIntegrationLifecycle:
    ...
    async def test_async_setup_entry_success(
        self, hass: HomeAssistant, mock_config_entry, mock_api_client, mock_coordinator
    ):
    ...
    async def test_async_setup_entry_api_exception(
        ...
        with patch(
            ...
            with pytest.raises(
                ConfigEntryNotReady, match="Failed to connect to Loca2 API"
            ):
                await async_setup_entry(hass, mock_config_entry)

    async def test_async_setup_entry_with_options(
        self, hass: HomeAssistant, mock_api_client, mock_coordinator
    ):
    ...
    async def test_async_setup_entry_default_values(
        self, hass: HomeAssistant, mock_api_client, mock_coordinator
    ):
    ...
    async def test_integration_setup_with_update_listener(
        self, hass: HomeAssistant, mock_config_entry, mock_api_client, mock_coordinator
    ):
    ...
    async def test_coordinator_first_refresh_failure(
        self, hass: HomeAssistant, mock_config_entry, mock_api_client, mock_coordinator
    ):
    ...
    async def test_hass_data_initialization(
        self, hass: HomeAssistant, mock_config_entry, mock_api_client, mock_coordinator
    ):
    ...
    async def test_multiple_entries_data_isolation(self, hass: HomeAssistant):
        ...
        with (
            patch(
                "custom_components.loca2.Loca2ApiClient",
                side_effect=[mock_api_client1, mock_api_client2],
            ),
            patch(
                "custom_components.loca2.Loca2DataUpdateCoordinator",
    ...
    async def test_unload_with_missing_data(
    ...
    async def test_unload_with_missing_domain_data(
        self, hass: HomeAssistant, mock_config_entry
    ):
    ...
    async def test_unload_with_missing_api_client(
        self, hass: HomeAssistant, mock_config_entry
    ):
    ...
    async def test_api_client_close_exception(
        self, hass: HomeAssistant, mock_config_entry
    ):
    ...
    async def test_setup_entry_stores_correct_data_structure(
        self, hass: HomeAssistant, mock_config_entry, mock_api_client, mock_coordinator
    ):
    ...
    async def test_setup_entry_platform_forward_failure(
        self, hass: HomeAssistant, mock_config_entry, mock_api_client, mock_coordinator
    ):
    ...
    async def test_update_listener_registration(
        ...
        with (
            ...
            mock_config_entry.async_on_unload.assert_called_once_with(
                mock_unload_listener
            )
