...
from .api import Loca2ApiClient, Loca2ApiError, Loca2AuthError, Loca2ConnectionError, Loca2RateLimitError, Loca2Device
from .const import (
    CONF_BASE_URL,
...
from .logging_utils import get_structured_logger, DiagnosticCollector, format_diagnostic_summary
...
class Loca2DataUpdateCoordinator(DataUpdateCoordinator[Dict[str, Loca2Device]]):
    ...
    def __init__(
        ...
        disabled_devices: Optional[list[str]] = None,
    ) -> None:
    ...
    async def _async_update_data(self) -> Dict[str, Loca2Device]:
        ...
        try:
            _LOGGER.debug("Fetching device data from Loca2 API (attempt %d)", 
            ...
            if self._disabled_devices:
                ...
                devices = [device for device in devices if device.id not in self._disabled_devices]
            ...
            await self._handle_error(err, ERROR_CATEGORY_API, "rate_limit", update_start_time)
            await self._handle_rate_limit()
            raise UpdateFailed(f"Rate limit exceeded: {err}") from err
            
        except Loca2AuthError as err:
            await self._handle_error(err, ERROR_CATEGORY_AUTH, "authentication", update_start_time)
            raise UpdateFailed(f"Authentication failed: {err}") from err
            
        except Loca2ConnectionError as err:
            await self._handle_error(err, ERROR_CATEGORY_NETWORK, "connection_error", update_start_time)
            self._consecutive_errors += 1
            await self._handle_api_error()
            raise UpdateFailed(f"Connection error: {err}") from err
            
        except Loca2ApiError as err:
            await self._handle_error(err, ERROR_CATEGORY_API, "api_error", update_start_time)
            self._consecutive_errors += 1
            await self._handle_api_error()
            raise UpdateFailed(f"API error: {err}") from err
            
        except Exception as err:
            await self._handle_error(err, ERROR_CATEGORY_UNKNOWN, "unexpected", update_start_time)
            self._consecutive_errors += 1
            await self._handle_api_error()
    ...
    async def async_get_device_location(self, device_id: str) -> Optional[Any]:
        """Get location for a specific device with error handling."""
        try:
            location = await self.api_client.get_device_location(device_id)
            return location
        except Loca2ApiError as err:
            await self._handle_error(err, ERROR_CATEGORY_API, "location_fetch", datetime.now(), device_id)
            _LOGGER.warning("Failed to get location for device %s: %s", device_id, err)
            return None
        except Exception as err:
            await self._handle_error(err, ERROR_CATEGORY_UNKNOWN, "location_fetch_unexpected", datetime.now(), device_id)
            _LOGGER.error("Unexpected error getting location for device %s: %s", device_id, err)
            return None

    async def _handle_successful_update(self, device_dict: Dict[str, Loca2Device], start_time: datetime) -> None:
        ...
        if was_in_error_state:
            _LOGGER.info(
                ...
                len(device_dict), update_duration
            )
        ...
        if update_duration > 10.0:  # Log slow updates
            _LOGGER.warning(
                ...
                update_duration, len(device_dict)
            )
    ...
    async def _handle_error(
        ...
        severity = self._determine_error_severity(category, error_type, self._consecutive_errors)
    ...
    async def _handle_error_notifications(self, error: Exception, category: str, error_type: str, severity: str) -> None:
        ...
        if last_sent and (now - last_sent).total_seconds() < NOTIFICATION_RATE_LIMIT_SECONDS:
            return
        
        # Only send notifications for medium severity and above
        if severity not in [ERROR_SEVERITY_MEDIUM, ERROR_SEVERITY_HIGH, ERROR_SEVERITY_CRITICAL]:
            return
        ...
        if category == ERROR_CATEGORY_AUTH:
            ...
            downtime_minutes = int((now - self._last_successful_update).total_seconds() / 60) if self._last_successful_update else 0
            await self._send_user_notification(
                NOTIFICATION_ID_CONNECTION_LOST,
    ...
    async def _send_user_notification(self, notification_id: str, title: str, message: str, severity: str = ERROR_SEVERITY_MEDIUM) -> None:
    ...
    async def _clear_error_notifications(self) -> None:
        ...
        for notification_id in notifications_to_clear:
            try:
                ...
                _LOGGER.debug("Could not clear notification %s: %s", notification_id, err)
    ...
    def update_configuration(self, scan_interval: int, disabled_devices: list[str]) -> None:
        """Update coordinator configuration."""
        self.adjust_scan_interval(scan_interval)
        self.update_disabled_devices(disabled_devices)
        _LOGGER.info("Configuration updated - scan_interval: %d, disabled_devices: %s", 
                    scan_interval, disabled_devices)
    ...
    def rate_limit_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_rate_limit": self._last_rate_limit.isoformat() if self._last_rate_limit else None,
    ...
    def get_diagnostic_info(self) -> Dict[str, Any]:
        ...
        if self._error_history:
            ...
            availability_percentage = max(0, (total_time_seconds - error_time_seconds) / total_time_seconds * 100)
        else:
        ...
        health_status = self._calculate_overall_health_status(availability_percentage, error_rate_1h)
    ...
    def log_diagnostic_summary(self) -> None:
        ...
        collector_diagnostics = self._diagnostic_collector.get_comprehensive_diagnostic()
        ...
        _LOGGER.info("  - Last update success: %s", diagnostics["coordinator"]["last_update_success"])
        ...
        if diagnostics["coordinator"]["last_exception"]:
            _LOGGER.info("  - Last exception: %s", diagnostics["coordinator"]["last_exception"])
        
        _LOGGER.info("Rate Limiting:")
        _LOGGER.info("  - Rate limit count: %d", diagnostics["rate_limiting"]["rate_limit_count"])
        _LOGGER.info("  - Current scan interval: %ds", diagnostics["rate_limiting"]["current_scan_interval"])
        _LOGGER.info("  - Consecutive errors: %d", diagnostics["rate_limiting"]["consecutive_errors"])
        
        _LOGGER.info("API Client:")
        _LOGGER.info("  - Connection status: %s", diagnostics["api_client"]["connection_status"])
    ...
    def _determine_error_severity(self, category: str, error_type: str, consecutive_errors: int) -> str:
    ...
    def _calculate_overall_health_status(self, availability: float, error_rate_1h: float) -> str:
        """Calculate overall health status based on multiple metrics."""
        from .const import HEALTH_STATUS_HEALTHY, HEALTH_STATUS_DEGRADED, HEALTH_STATUS_UNHEALTHY
        ...
        if (self._consecutive_errors >= 3 or 
            error_rate_1h >= 10 or 
            availability < 90.0 or 
            self._backoff_multiplier > 2):
    ...
    def _analyze_error_trends(self) -> Dict[str, Any]:
        ...
        previous_errors = [
            error for error in self._error_history
            if two_hours_ago < datetime.fromisoformat(error["timestamp"]) <= one_hour_ago
        ]
    ...
    def _calculate_average_update_duration(self) -> Optional[float]:
    ...
    def should_log_diagnostic_summary(self) -> bool:
        ...
        if self._error_history and (datetime.now() - self._last_diagnostic_log).total_seconds() > 300:
            return True
    ...
    async def perform_health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            data_fresh = True
            if self._last_successful_update:
                data_age = (datetime.now() - self._last_successful_update).total_seconds()
            ...
            overall_healthy = (api_healthy and data_fresh and 
                             error_rate_acceptable and consecutive_errors_ok)
            ...
            health_status = {
                ...
                "metrics": {
                    "error_rate_1h": error_rate_1h,
                    "consecutive_errors": self._consecutive_errors,
                    "data_age_seconds": (datetime.now() - self._last_successful_update).total_seconds() if self._last_successful_update else None,
                    "scan_interval": self._scan_interval,
                },
                "timestamp": datetime.now().isoformat(),
...
async def async_update_options(hass: HomeAssistant, entry: ConfigEntry) -> None:
    ...
    if DOMAIN not in hass.data or entry.entry_id not in hass.data[DOMAIN]:
        _LOGGER.warning("Coordinator not found for options update, performing full reload")
    ...
    new_scan_interval = entry.options.get(CONF_SCAN_INTERVAL, entry.data.get(CONF_SCAN_INTERVAL, DEFAULT_SCAN_INTERVAL))
