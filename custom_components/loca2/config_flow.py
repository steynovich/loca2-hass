...
from .api import Loca2ApiClient, Loca2ApiError, Loca2AuthError, Loca2ConnectionError
from .const import (
    CONF_BASE_URL,
...
async def validate_input(hass: HomeAssistant, data: dict[str, Any]) -> dict[str, Any]:
    ...
    # Test the connection
    client = Loca2ApiClient(
        account=username, password=password, base_url=base_url, timeout=timeout
    )
    ...
    try:
        async with client:
            ...
            _LOGGER.debug(
                "Successfully connected to Loca2 API, found %d devices", len(devices)
            )
...
class Loca2OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    def __init__(self, config_entry: config_entries.ConfigEntry) -> None:
        ...
        self._available_devices: dict[str, str] | None = None
    ...
    async def async_step_init(
        ...
        current_timeout = self._config_entry.options.get(
            CONF_TIMEOUT, self._config_entry.data.get(CONF_TIMEOUT, DEFAULT_TIMEOUT)
        )
        ...
        options_schema = vol.Schema(
            {
                ...
                vol.Optional(CONF_TIMEOUT, default=current_timeout): vol.All(
                    cv.positive_int, vol.Range(min=1, max=120)
                ),
            }
        )
        ...
        if self._available_devices:
            ...
            options_schema = options_schema.extend(
                {
                    vol.Optional(
                        CONF_DISABLED_DEVICES, default=current_disabled_devices
    ...
    async def _get_available_devices(self) -> None:
        ...
        try:
            ...
            async with client:
                ...
                _LOGGER.debug(
                    "Found %d devices for options", len(self._available_devices)
                )
        ...
        try:
            ...
            if scan_interval is not None:
                if not isinstance(scan_interval, int):
                    validation_errors.append(
                        f"Scan interval must be an integer, got {type(scan_interval).__name__}"
                    )
                elif (
                    ...
                    validation_errors.append(
                        f"Scan interval must be between {MIN_SCAN_INTERVAL} and {MAX_SCAN_INTERVAL} seconds, got {scan_interval}"
                    )
                else:
                    _LOGGER.debug(
                        "Scan interval validation passed: %d seconds", scan_interval
                    )
            ...
            if timeout is not None:
                if not isinstance(timeout, int):
                    ...
                    validation_errors.append(
                        f"Timeout must be between 1 and 120 seconds, got {timeout}"
                    )
                else:
            ...
            if disabled_devices:
                if not isinstance(disabled_devices, list):
                    validation_errors.append(
                        f"Disabled devices must be a list, got {type(disabled_devices).__name__}"
                    )
                else:
                    ...
                    if self._available_devices:
                        ...
                        if invalid_devices:
                            validation_errors.append(
                                f"Unknown devices: {', '.join(invalid_devices)}"
                            )
                        else:
                            _LOGGER.debug(
                                "Disabled devices validation passed: %s",
                                disabled_devices,
                            )
                    else:
                        _LOGGER.warning(
                            "Cannot validate disabled devices - available devices list not loaded"
                        )

            # Log validation summary
            if validation_errors:
                _LOGGER.warning(
                    "Options validation failed with %d errors: %s",
                ...
                _LOGGER.info(
                    "Options validation successful for %d parameters", len(user_input)
                )
