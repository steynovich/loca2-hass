...
async def async_setup_entry(
    ...
    def _async_add_new_devices():
        ...
        if new_entities:
            _LOGGER.info(
                "Adding %d new Loca2 device tracker entities", len(new_entities)
            )
...
class Loca2DeviceTracker(CoordinatorEntity, TrackerEntity):
    ...
    def __init__(
        ...
        self._device = device
        self._location: Loca2Location | None = None
    ...
    @property
    def device(self) -> Loca2Device | None:
    ...
    def icon(self) -> str:
        ...
        # model = (device.model or "").lower()  # Not used currently
    ...
    async def _update_device_location(self) -> None:
        ...
        try:
            with self._structured_logger.operation_timer("location_fetch"):
                location = await self.coordinator.async_get_device_location(
                    self._device_id
                )
            ...
            if self._consecutive_location_errors >= 10:
                from .const import ERROR_SEVERITY_HIGH
            ...
            self._structured_logger.log_error(
                ...
                extra_data={
                    ...
                    "coordinator_error_count": getattr(
                        self.coordinator, "_consecutive_errors", 0
                    ),
                },
            )
    ...
    def get_device_diagnostics(self) -> dict[str, Any]:
        ...
        return {
            ...
            "location_info": {
                "has_location": self._location is not None,
                "location_quality": (
                    self._assess_location_quality(self._location)
                    if self._location
                    else None
                ),
                "coordinates_valid": (
                    self._location.is_valid_coordinates() if self._location else False
                ),
            ...
            "device_info": {
                ...
                "last_seen": (
                    self.device.last_seen.isoformat()
                    if self.device and self.device.last_seen
                    else None
                ),
