...
class Loca2Device:
    ...
    def _validate_data(self) -> None:
        ...
        if self.battery_level is not None:
            if not isinstance(self.battery_level, int) or not (0 <= self.battery_level <= 100):
    ...
    def from_dict(cls, data: Dict[str, Any]) -> Loca2Device:
        ...
        location_time = cls._convert_timestamp(spot_data.get("time")) if spot_data else None
        ...
        # Status information from History
        battery_level = history_data.get("charge") if history_data else None
        last_seen = cls._convert_timestamp(history_data.get("time")) if history_data else None
    ...
    @staticmethod
    def _convert_battery_level(value: Any) -> Optional[int]:
        ...
        try:
            ...
            if not (0 <= battery_int <= 100):
                _LOGGER.warning("Battery level %d is outside valid range (0-100), clamping", battery_int)
    ...
    @staticmethod
    def _convert_datetime(value: Any, field_name: str) -> Optional[datetime]:
        ...
        if not isinstance(value, str):
            _LOGGER.warning("Invalid %s format (not string): %s", field_name, type(value))
            return None
    ...
    @staticmethod
    def _get_device_type_from_id(type_id: Optional[int]) -> str:
...
class Loca2Location:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> Loca2Location:
        ...
        if "latitude" not in data or "longitude" not in data:
            raise ValueError("Location data must contain 'latitude' and 'longitude' fields")
    ...
    @staticmethod
    def _convert_coordinate(value: Any, coord_name: str, min_val: float, max_val: float) -> float:
        ...
        try:
            ...
            if not (min_val <= coord_float <= max_val):
                raise ValueError(f"{coord_name} {coord_float} is outside valid range ({min_val} to {max_val})")
    ...
    @staticmethod
    def _convert_accuracy(value: Any) -> Optional[float]:
        ...
        try:
            ...
            if accuracy_float < 0:
                _LOGGER.warning("Negative accuracy value %f, setting to 0", accuracy_float)
                accuracy_float = 0.0
    ...
    @staticmethod
    def _convert_address(value: Any) -> Optional[str]:
...
class Loca2ApiClient:
    ...
    async def _make_request(
        ...
        **kwargs: Any,
        ...
        _LOGGER.debug("Making %s request to %s (attempt 1/%d)", method, endpoint, DEFAULT_RETRIES)
        ...
        for attempt in range(DEFAULT_RETRIES):
            ...
            try:
                    ...
                    async with session.request(
                        ...
                        if response_time > 5.0:  # Slow response threshold
                            _LOGGER.warning(
                                ...
                                method, endpoint, response_time, response.status, attempt + 1, DEFAULT_RETRIES
                            )
                        else:
                            _LOGGER.debug(
                        ...
                        if response.status == 401:
                            ...
                            self._error_count += 1
                            _LOGGER.error("Authentication failed for %s %s", method, endpoint)
                            ...
                            raise Loca2RateLimitError(f"Rate limit exceeded (retry after: {retry_after})")
                            ...
                            _LOGGER.error(
                                ...
                                method, endpoint, response.status, error_text
                            )
                            raise Loca2ApiError(
                        ...
                        try:
                            ...
                            _LOGGER.debug("Successfully parsed JSON response for %s %s", method, endpoint)
                            return response_data
                        except Exception as json_err:
                            self._last_error = f"JSON parsing error: {json_err}"
                            self._error_count += 1
                            _LOGGER.error("Failed to parse JSON response for %s %s: %s", method, endpoint, json_err)
                            raise Loca2ApiError(f"Failed to parse JSON response: {json_err}") from json_err
                ...
                _LOGGER.warning(
                    ...
                    method, endpoint, attempt + 1, DEFAULT_RETRIES, response_time
                )
                ...
                if attempt == DEFAULT_RETRIES - 1:
                    ...
                    _LOGGER.error(
                        ...
                        method, endpoint, total_time
                    )
                    raise Loca2ConnectionError(f"Request timeout after {total_time:.1f}s") from err
                ...
                _LOGGER.warning(
                    ...
                    method, endpoint, attempt + 1, DEFAULT_RETRIES, str(err)
                )
                ...
                if attempt == DEFAULT_RETRIES - 1:
                    ...
                    _LOGGER.error(
                        ...
                        method, endpoint, total_time, str(err)
                    )
            ...
            if attempt < DEFAULT_RETRIES - 1:
                ...
                _LOGGER.debug(
                    ...
                    method, endpoint, retry_delay, attempt + 2, DEFAULT_RETRIES
                )
        ...
        _LOGGER.error("Max retries exceeded for %s %s (total time: %.1fs)", method, endpoint, total_time)
            ...
            async with async_timeout.timeout(self._timeout):
                async with session.post(url, data=auth_data) as response:
                    if response.status == 401:
                        ...
                        error_text = await response.text()
                        raise Loca2ApiError(f"Authentication failed with status {response.status}: {error_text}")
                    
                    # Extract session cookie
                    if "sid" in response.cookies:
                        self._sid_cookie = response.cookies["sid"].value
                        _LOGGER.debug("Successfully authenticated and received session cookie")
                    else:
                        raise Loca2AuthError("No session cookie received after authentication")
    ...
    async def get_devices(self) -> List[Loca2Device]:
        ...
        try:
            ...
            if not isinstance(response, list):
                _LOGGER.warning("Expected list response from asset status API, got: %s", type(response))
                return []
            ...
            for asset_status in response:
                try:
                    ...
                    _LOGGER.warning("Invalid asset status data: %s - %s", asset_status, err)
                    continue
    ...
    async def get_device_location(self, device_id: str) -> Loca2Location:
        ...
        try:
            ...
            for device in devices:
                if device.id == device_id:
                    if device.latitude is not None and device.longitude is not None:
                        ...
                        raise Loca2ApiError(f"No location data available for device {device_id}")
            ...
            raise Loca2ApiError(f"Failed to get location for device {device_id}: {err}") from err
    ...
    def get_diagnostic_info(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "last_response_time": f"{self._last_response_time:.3f}s" if self._last_response_time else None,
            "average_response_time": f"{avg_response_time:.3f}s" if avg_response_time else None,
    ...
    def _calculate_health_status(self, success_rate: float) -> str:
        ...
        elif success_rate >= 80.0 and self._connection_status in ["connected", "api_error"]:
